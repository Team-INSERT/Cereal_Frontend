...
const ChatBox = () => {
	const [randomChat, setRandomChat] = useState<string[]>([])

	useEffect(() => {
		setRandomChat([...distinct()])
	}, [])
	...
	return (
		<S.ChatBoxWrap>
			...
			<S.ChatRecommendWrap>
				...
				{randomChat.map((chat, index) => (
					<S.ChatBox color={index % 2 == 0 ? '#007aff' : '#aaa'} key={index}>
						<S.ChatText>{chat}</S.ChatText>
					</S.ChatBox>
				))}
				<svg
					onClick={() => setRandomChat([...distinct()])}
					width="16"
					height="16"
					viewBox="0 0 16 16"
					fill="none"
